...
const btns = document
  ...
  .forEach(btn => btn.addEventListener("click", btnToggle));
...
function hiddeChannels(state) {
  ...
  const channels = [...document.querySelectorAll(".channel")].filter(
    channel => channel.dataset.state === state
  );
    ...
    $.getJSON(
      ...
      response => {
        if (response.stream) {
          ...
          getChannelData(
            `https://wind-bow.gomix.me/twitch-api/channels/${user}?callback=?`
          );
        }
      }
    );
  ...
  renderArea.innerHTML += `
    <div class="channel ${state}" data-state=${state}>        
        ...
        <div class="channel-info">
          <a class="channel-name" target="_blank" href="${link}">${name}</a>
